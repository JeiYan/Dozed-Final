...
$xPos = $pageWidth - $logoWidth - 5;

$pdf->Image('../image/logo.png', $xPos, 6, $logoWidth);
...
$pdf->Cell(0, 10, "DOZED'S GREAT COFFEE | GOOD COFFEE", 0, 1, 'C');
...
while ($item_row = mysqli_fetch_assoc($items_result)) {
    ...
    $pdf->Cell(40, 10, 'PHP ' . number_format($item_price,2), 1);
    $pdf->Cell(40, 10, $quantity, 1);
...
$pdf->Cell(40, 10, 'PHP ' . number_format($before_tax,2), 1);
$pdf->Ln();

$pdf->Cell(150, 10, 'Tax (' . ($tax_rate * 100) . '%)', 1);
$pdf->Cell(40, 10, 'PHP ' . number_format($tax_amount,2), 1);
$pdf->Ln();

$pdf->Cell(150, 10, 'Grand Total', 1);
$pdf->Cell(40, 10, 'PHP ' . number_format($after_tax,2), 1);
$pdf->Ln();

$pdf->Ln();  // Add extra space before the message
    $pdf->SetFont('Arial', 'I', 12); // Italics for the message
    $pdf->Cell(0, 10, 'Please present this receipt at the counter to proceed with your Reservation.', 0, 'C');
