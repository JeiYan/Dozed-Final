            ...
            <div class="col-md-15" style="margin-left: 17rem; margin-top: 0rem;max-height: 700px; overflow-y: auto;">
                ...
                if ($reservationResult && mysqli_num_rows($reservationResult) > 0) {
                    echo '<a href="orderItem.php?bill_id=' . $latestBillID . '&table_id=' . $table_id . '" class="btn btn-primary btn-block btn-lg" style="color:black; '
                        ...
                        . 'Table: ' . $table_id . '<br>Capacity: ' . $capacity . '</a>';
                // 🔴 Occupied (bill exists, not paid)
                    } elseif ($latestBillID && !$hasPaymentTime) {
                        echo '<a href="orderItem.php?bill_id=' . $latestBillID . '&table_id=' . $table_id . '" class="btn btn-primary btn-block btn-lg" style="color:white; '
                            . 'background-color: rgb(216, 0, 50); justify-content: center; align-items: center; display: flex; width: 9rem; height: 9rem;">'
                            . 'Table: ' . $table_id . '<br>Capacity: ' . $capacity . '</a>';
                    
                // 🟢 Available (no bill or bill is paid)
                    } else {
                        echo '<a href="orderItem.php?bill_id=' . $latestBillID . '&table_id=' . $table_id . '" class="btn btn-primary btn-block btn-lg" style="color:white; '
                        . 'background-color: rgb(23, 89, 74); justify-content: center; align-items: center; display: flex; width: 9rem; height: 9rem;">'
                        . 'Table: ' . $table_id . '<br>Capacity: ' . $capacity . '</a>';
                }
                        echo '</a></div>';
                        $table_count++;
